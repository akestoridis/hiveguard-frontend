 ...
 * hiveguard-frontend/src/components/pages/PacketCounters.jsx
 * @license Apache-2.0
 */
...
function PacketCounters({ inspectionAPI }) {
  ...
  useEffect(
    () => {
      if (inspectionAPI && selSensorState && selIntvlState && selPANState) {
        setPANURLState(
          `${inspectionAPI}/packet-counters/?sensor=${selSensorState}`
  ...
  return (
    <Container fluid>
      <div className="p-5 mb-4 rounded-3 bg-light text-dark text-center">
        <h1 className="display-5 fw-bold">Packet Counters Page</h1>
        <br />
        <Container>
          <Row
      ...
      <Row className="row-cols-1 row-cols-lg-2">
        <Col>
          <TimeBarChart
            dataURL={panURLState}
            dataLabel="New PAN Packets over Time"
        ...
        <Col>
          <TimeBarChart
            ...
            dataLabel="New Device Packets over Time"
...
PacketCounters.propTypes = {
  inspectionAPI: PropTypes.string.isRequired,
};

export default PacketCounters;
